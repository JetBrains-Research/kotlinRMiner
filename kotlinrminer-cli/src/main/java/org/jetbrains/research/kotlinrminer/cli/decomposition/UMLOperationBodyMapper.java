...
import org.jetbrains.research.kotlinrminer.cli.decomposition.replacement.*;
import org.jetbrains.research.kotlinrminer.cli.diff.*;
import org.jetbrains.research.kotlinrminer.cli.diff.refactoring.CandidateAttributeRefactoring;
import org.jetbrains.research.kotlinrminer.cli.diff.refactoring.CandidateMergeVariableRefactoring;
import org.jetbrains.research.kotlinrminer.cli.diff.refactoring.CandidateSplitVariableRefactoring;
...
import org.jetbrains.research.kotlinrminer.cli.util.ReplacementUtil;
...
/*    TODO:    if (statement1 instanceof TryStatementObject && statement2 instanceof TryStatementObject) {
    ...
    private String prepareConditional(String s) {
        ...
        if (indexOfEquals > 0 && indexOfEquals < s.length() -1 &&
            s.charAt(indexOfEquals + 1) != '=' && s.charAt(indexOfEquals - 1) != '!') {
            conditional = s.substring(indexOfEquals + 1, s.length());
        }
        return conditional;
    }
            ...
            if (s1.contains(element1) && !s1.equals(element1) && !s1.equals("this." + element1) && !s1.equals(
                ...
                for (String element2 : set2) {
                    ...
                    if (s2.contains(element2) && !s2.equals(element2)) {
                        ...
                        if (direction.equals(VariableReplacementWithMethodInvocation.Direction.VARIABLE_TO_INVOCATION))
                            methodInvocationList = methodInvocationMap.get(element2);
                        else if (direction.equals(VariableReplacementWithMethodInvocation.Direction.INVOCATION_TO_VARIABLE))
